...
const ReportTextList = ({
  ...
  addLabel = 'إضافة',
  emptyHint = 'لم تُضف عناصر بعد. اضغط «إضافة» لإضافة عنصر جديد.',
  suggestions = [],
  readOnly = false,
}) => {
  ...
  return (
    <div className="report-text-list">
      ...
      {items.length === 0 ? (
        <p className="report-text-list__empty">{emptyHint}</p>
      ) : (
        <ul className="report-text-list__items">
          {items.map((item, index) => (
            <li key={`${index}-${item}`} className="report-text-list__item">
              ...
              />
              <button type="button" className="icon-btn" title="حذف" onClick={() => removeItem(index)}>
